---
name: annotate
description: rune 어노테이션을 컴포넌트에 자동으로 추가하는 스킬. 사용자가 "어노테이션 달아줘", "이 컴포넌트를 AI가 제어할 수 있게 해줘", "rune 어노테이션", "annotate", "data-rune 추가", "이 페이지 전체에 어노테이션", "프로젝트 전체 어노테이션" 등을 말하면 반드시 이 스킬을 사용할 것. 컴포넌트나 페이지의 인터랙티브 요소를 분석하고 적절한 data-rune-* 속성을 제안/적용하는 모든 작업에 이 스킬이 필요하다.
---
...
어노테이션이 동작하려면 앱의 `vite.config.ts`에 rune 플러그인이 등록되어 있어야 한다.
어노테이션을 추가하기 전에 확인하고, 없으면 사용자에게 알린다.

```ts
// vite.config.ts
import runeDomPlugin from '@runeai/build-core'
