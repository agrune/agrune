// In-page functions executed via locator.evaluate / locator.evaluateAll. Playwright serializes
// the function source, so each is fully self-contained (no module-scope references survive).
//
// captureElementState is the single source of truth for the DOM heuristics — visibility,
// viewport clipping, covered, overlay, and SENSITIVE masking (core security fail-safe, A.0.5 /
// §8.4). Reproduced from the original page-functions.ts (canvas transform reader dropped — §10.2).
...
export function expandRepeatRows(elements: Element[], args: RepeatRowArgs): RepeatRow[] {
  ...
  try {
    ...
    /* compile failed → fall back to index keys */
  }
...
/** READ-ONLY logical-size hint from a virtualized container: aria-rowcount > aria-setsize. */
