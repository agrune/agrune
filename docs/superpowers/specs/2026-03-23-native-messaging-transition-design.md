...
- native host 연결 상태 배지와 수동 reconnect 제공
...
- native host는 얇은 wrapper로 두고, wrapper와 manifest는 설치기가 자동 생성
...
- 메시지 타입:
  ...
  - `ping` / `pong`: native host 연결 확인
  - `get_status` / `status_response`: native host 상태 조회/응답

**Extension 내부 메시지**:
- `config_broadcast`: popup → service worker (설정 저장 후 브로드캐스트)
- `get_native_host_status`: popup → service worker (현재 상태 조회)
- `reconnect_native_host`: popup → service worker (수동 재연결)
- `native_host_status_changed`: service worker → popup/content scripts (상태 브로드캐스트)
...
1. `webcli-mcp install` 실행
2. Chrome에서 extension 확인/활성화 프롬프트를 승인
3. Native Messaging Host 설정 파일 자동 생성
