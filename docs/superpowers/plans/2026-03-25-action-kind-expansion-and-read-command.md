...
| `packages/mcp/src/tools.ts` | MCP 툴 스키마 (레거시) | agrune_act에 action 파라미터, agrune_read 추가 |
| `packages/mcp/src/mcp-tools.ts` | MCP 툴 등록 (Zod) | agrune_act에 action 파라미터, agrune_read 등록 |
| `packages/mcp/src/backend.ts` | 백엔드 핸들러 | switch에 agrune_read 추가 |
| `packages/build-core/src/runtime/page-agent-runtime.ts` | 런타임 | 인터페이스 확장, 가드 로직, 이벤트 헬퍼, 시퀀스 함수, read 핸들러 |
| `packages/mcp/tests/tools.spec.ts` | 툴 정의 테스트 | 새 툴/파라미터 검증 |
| `packages/mcp/tests/backend.spec.ts` | 백엔드 테스트 | agrune_read 통합 테스트 |
...
**Files:**
- Modify: `packages/mcp/src/tools.ts:48-58` (agrune_act), 끝에 agrune_read 추가
- Modify: `packages/mcp/src/mcp-tools.ts:46-54` (agrune_act), 끝에 agrune_read 추가
- Test: `packages/mcp/tests/tools.spec.ts`

- [ ] **Step 1: 실패 테스트 작성 — tools.spec.ts에 새 검증 추가**

`packages/mcp/tests/tools.spec.ts`에 기존 테스트 수정 및 추가:
...
`packages/mcp/src/tools.ts` agrune_act 정의 (line 48-58):
...
`packages/mcp/src/tools.ts` agrune_config 다음에 추가:
...
`packages/mcp/src/mcp-tools.ts` agrune_act 등록 (line 46-54):
...
`packages/mcp/src/mcp-tools.ts` agrune_config 등록 다음에 추가:
...
```bash
cd /Users/chenjing/dev/agrune && git add packages/mcp/src/tools.ts packages/mcp/src/mcp-tools.ts packages/mcp/tests/tools.spec.ts && git commit -m "feat: add action parameter to agrune_act and new agrune_read tool definition"
```
...
**Files:**
- Modify: `packages/mcp/src/backend.ts:90-108` (switch 케이스)
...
`packages/mcp/src/backend.ts` line 93-94 사이에 `'agrune_read'` 추가:
...
`packages/mcp/tests/backend.spec.ts`에 추가 (기존 describe 블록 내부):
...
```bash
cd /Users/chenjing/dev/agrune && git add packages/mcp/src/backend.ts packages/mcp/tests/backend.spec.ts && git commit -m "feat: route agrune_read through command pipeline"
```
