...
**Architecture:** `packages/mcp/package.json`에 `postbuild` 스크립트 추가. `~/.agrune/mcp-server/`가 `dist/`의 flat copy 구조이므로 `dist/*` → `~/.agrune/mcp-server/` 복사 후 포트 47654 데몬 kill.
...
| 파일 | 변경 | 역할 |
|------|------|------|
| `packages/mcp/package.json` | 수정 | `postbuild` 스크립트 추가 |
| `packages/mcp/scripts/postbuild.sh` | 생성 | 복사 + 데몬 kill 로직 |
...
**Files:**
- Create: `packages/mcp/scripts/postbuild.sh`
...
Run: `chmod +x packages/mcp/scripts/postbuild.sh`
...
**Files:**
- Modify: `packages/mcp/package.json:6-11`
...
```bash
git add packages/mcp/scripts/postbuild.sh packages/mcp/package.json README.md
