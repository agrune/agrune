...
- `rune_act`, `rune_fill`, `rune_drag`, `rune_guide` 실행 전에 선택적으로 탭을 활성화할 수 있으면 실사용성이 크게 오른다.
...
- 팝업 또는 side panel에서 현재 열려 있는 `rune` 세션 목록을 보여주고, "현재 제어 대상"을 명시적으로 고를 수 있어야 한다.
...
- 어노테이션 헬퍼: 선택한 DOM 요소에 대해 `data-rune-*` 후보, selector, 이름 추천을 제공
