...
- **#5 CDP 실제 연결** — ✅ 해결. cdpPostMessage 콜백 주입 + content script relay 수정.
- **#1 카드 드래그 애니메이션** — ✅ CDP로 자동 해결.
- **#2 칸반 카드 이동** — ✅ CDP로 자동 해결.
- **#4 캔버스 줌** — ✅ CDP로 자동 해결.
- **#7 워크플로우 노드 드래그** — ✅ CDP로 자동 해결.
- **#8 합성 fallback 제거** — 🔧 진행 중.

## 추가 개선사항

### 9. pointer 액션 간 딜레이 지원

**현상:** wheel 줌 등 여러 단계를 부드럽게 표현하려면 액션을 여러 번 수동으로 나열해야 함. 비효율적.

**해결 방향:** pointer 액션에 `delayMs` 필드 추가하거나, `smooth: true` 모드로 시작/끝만 지정하면 중간 스텝 자동 생성.

### 10. 캔버스 노드 좌표 계산 정확도

**현상:** React Flow 캔버스에서 노드를 정렬할 때 뷰포트 좌표와 캔버스 내부 좌표가 1:1 매핑되지 않아 배치가 삐딱해짐. 줌/팬 상태에 따라 좌표 변환 필요.

**해결 방향:** 캔버스 transform(scale, translateX/Y)을 읽어서 뷰포트 좌표 → 캔버스 좌표 변환하는 로직 추가. 또는 스냅샷에 캔버스 transform 정보를 포함.
