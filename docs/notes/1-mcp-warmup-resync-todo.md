...
- 이 상태에서 cold start 직후 첫 `rune_snapshot` 호출은 빈 결과나 에러를 돌려줄 수 있다.

## 목표

- `rune_snapshot` 첫 호출에서 빈 스냅샷을 바로 반환하지 않는다.
...
6. 준비가 끝나면 그때 `rune_snapshot` 또는 후속 command 응답을 반환한다.

즉 warm-up은 "빈 캐시를 들고 기다리는 시간"이 아니라 "브라우저 상태 캐시를 다시 채우는 준비 단계"로 정의해야 한다.

## 왜 느리게 느껴지는가

- 현재 `rune_snapshot`은 snapshot이 없으면 바로 실패한다.
...
4. `rune_snapshot` 진입 전에 `ensureReady` 같은 준비 단계를 두고, 짧은 시간 동안 warm-up 완료를 기다리게 한다.
5. `rune_act`, `rune_fill`, `rune_drag`, `rune_wait`도 같은 준비 단계를 타야 하는지 검토한다.
...
- `rune_snapshot` 앞단에 `ensureReady`를 두고, 짧은 시간 동안 warm-up 완료를 기다린다.
...
- `rune_snapshot`의 의미를 "현재 캐시를 그대로 보여준다"가 아니라 "사용 가능한 snapshot을 준비해 반환한다"로 바꾼다.
- `rune_act`, `rune_fill`, `rune_drag`, `rune_wait`도 필요 시 같은 준비 단계를 거치도록 통일한다.
